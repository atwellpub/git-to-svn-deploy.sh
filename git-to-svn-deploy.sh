...
# main config
PLUGINSLUG="import-liked-youtube-videos"
CURRENTDIR=`pwd`
MAINFILE="import-liked-youtube-videos.php" # this should be the name of your main php file in the wordpress plugin
...
SVNURL="http://plugins.svn.wordpress.org/import-liked-youtube-videos/" # Remote SVN repo on wordpress.org, with no trailing slash
SVNUSER="Hudson Atwell" # your svn username
SVNPASS="" # your svn username
...
#Push to Bitbucket
if git remote | grep bitbucket > /dev/null; then
	echo "Pushing latest commit to origin 'bitbucket', with tags"
...
ls
svn commit --username="$SVNUSER" --password="$SVNPASS"  -m "Tagging version $NEWVERSION1" --no-auth-cache --force-log
sleep 10
